...
app.http("server-rankings", {
  ...
  handler: async (request, context) => {
    try {
      ...
      const data = {
        revenueRanking: rankRows(rows, "total_revenue", "desc", 10),
        visitCountRanking: rankRows(rows, "visit_count", "desc", 10),
        averageTipPercentRanking: rankRows(rows, "avg_tip_percent", "desc", 10),
        formulas: {
